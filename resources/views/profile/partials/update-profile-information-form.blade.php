<section>
    <header>
        ...
        <p class="mt-1 text-muted">
            {{ __("Modifica le tue informazioni e l'indirizzo email") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div class="mb-2">
            ...
            <input class="form-control" type="text" name="lastname" id="lastname" autocomplete="lastname"
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && !$user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-muted">
                        {{ __('La tua email non e coretta.') }}

                        <button form="send-verification" class="btn btn-outline-dark">
                            {{ __('Clicca qui per rimadarla.') }}
                        </button>
                    </p>
                    ...
                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 text-success">
                            ...
                            {{ __('E stata mandata un link di verifica alla tua Email') }}
